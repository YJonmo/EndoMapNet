...
#                    #print(pred_poses)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.use_pose=='1':
            ...
            translation_GtMag = torch.zeros(self.opt.batch_size, frame_lengh, requires_grad=True, dtype=torch.float32) 
            translation_GtMag = translation_GtMag.to(device='cuda')
            translation_PrMag = torch.zeros(self.opt.batch_size, frame_lengh, requires_grad=True, dtype=torch.float32) 
            translation_PrMag = translation_PrMag.to(device='cuda')
            axisangle_GtMag = torch.zeros(self.opt.batch_size, frame_lengh, requires_grad=True, dtype=torch.float32) 
            axisangle_GtMag = axisangle_GtMag.to(device='cuda')
            axisangle_PrMag = torch.zeros(self.opt.batch_size, frame_lengh, requires_grad=True, dtype=torch.float32) 
            ...
            for Bach_ind in range(self.opt.batch_size):
                for Frame_ind in range(frame_lengh):
                    translation_gt[Bach_ind, :, Frame_ind] *= self.opt.trans_weight
                    translation_pred[Bach_ind, :, Frame_ind] *= self.opt.trans_weight
                        
                    translation_GtMag[Bach_ind,Frame_ind]=torch.norm(translation_gt[Bach_ind,:,Frame_ind].clone(),dim=0)
                    translation_PrMag[Bach_ind, Frame_ind]= torch.norm(translation_pred[Bach_ind, :, Frame_ind].clone(), dim=0)
                    axisangle_GtMag[Bach_ind, Frame_ind]= torch.norm(axisangle_gt[Bach_ind, :, Frame_ind].clone(), dim=0)
                    axisangle_PrMag[Bach_ind, Frame_ind]= torch.norm(axisangle_pred[Bach_ind,:,Frame_ind].clone(), dim=0) 
            
            losses2 = {}
            Coeff =  1000
            losses2["loss2_Normal"]=self.criterion(F.normalize(translation_gt),F.normalize(translation_pred))/(Coeff)
            losses2["loss2_Normal_Mag"]=self.criterion(F.normalize(translation_GtMag, dim=0),F.normalize(translation_PrMag, dim=0))/(Coeff/4)      
            ...
            losses2["loss2"]=(losses2["loss2_Normal"]+losses2["loss2_Normal_Mag"]+
                losses2["loss2_Normal_Ang"]+losses2["loss2_Normal_Ang_Mag"])
            #losses2["loss2"]=(losses2["loss2_Normal"]+losses2["loss2_Normal_Mag"])
