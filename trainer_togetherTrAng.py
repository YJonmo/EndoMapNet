    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.use_pose=='1':
            ...
            if  torch.mean(inputs[("translation", self.opt.frame_ids[1], 0)]) == 0:     # if the ground truth pose was not available then use the loss of the preveious run
                ...
                print('It was hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') 
                losses2["loss2"] = self.last_loss.item() + (losses2["loss2_Normal"] + losses2["loss2_Normal_Ang"] )/(frame_lengh*10000);
            else:
                ...
                losses2["loss2_Normal_Ang"]=self.criterion(F.normalize(axisangle_gt), F.normalize(axisangle_pred))/(15*frame_lengh) * 0.25  
                losses2["loss2"] = (losses2["loss2_Normal"] + losses2["loss2_Normal_Ang"] )
