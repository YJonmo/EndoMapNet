        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                if self.opt.use_pose=='1':
                    ...
                    self.models["pose2"] = networks.PoseDecoderQuat(
...
#            self.opt.frame_ids, 4, is_train=True, img_ext=img_ext)
        train_dataset = self.dataset(
            self.opt.data_path, train_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, 4, is_train=True, img_ext=img_ext, use_pose=self.opt.use_pose, use_quat=self.opt.use_quat)
        ...
        if self.opt.use_pose=='1':
            ...
            val_dataset = self.dataset(
                ...
                self.opt.frame_ids, 4, is_train=False, img_ext=img_ext, use_pose=self.opt.use_pose, use_quat=self.opt.use_quat)
    ...
    def process_batch2(self, inputs):
        ...
        axisangle_gt = torch.zeros(self.opt.batch_size, 4, frame_lengh, requires_grad=True, dtype=torch.float64) 
        axisangle_gt = axisangle_gt.to(device='cuda')        
        axisangle_pred = torch.zeros(self.opt.batch_size, 4, frame_lengh, requires_grad=False, dtype=torch.float64) 
        ...
        for frame_id in (self.opt.frame_ids[1:]):
            if frame_id != "s":
                ...
                translation_pred[:, :, i] = outputs2[("translation", 0, frame_id)][:,:]*1000
                axisangle_pred[:, :, i] = outputs2[("axisangle", 0, frame_id)][:,:]*1000
                axisangle_gt[:, :, i] = inputs[("axisangle", frame_id, 0)]*(1/1)
                axisangle_gt[:, 0, i] = (axisangle_gt[:, 0, i] - 1)*100
                axisangle_gt[:, :, i] = axisangle_gt[:, :, i]*100
                translation_gt[:, :, i] = inputs[("translation", frame_id, 0)]*(1/1)

                i += 1
        print('axisangle_gt[0,:,i-1].abs().mean(): ')                
        print(axisangle_gt[0,:,i-1].data) 
        print('axisangle_pred[0,:,i-1].abs().mean(): ')
...
#        
        ...
        if  torch.mean(inputs[("translation", self.opt.frame_ids[1], 0)]) == 0:     # if the ground truth pose was not available then use the loss of the preveious run
            ...
            losses2["loss2_Normal_Ang"]=self.criterion(F.normalize(axisangle_pred), F.normalize(axisangle_pred))  * 0.25     
            losses2["loss2_Raw_Ang"] = self.criterion(axisangle_pred, axisangle_pred) * 0.5
            ...
            losses2["loss2_Normal_Ang"]=self.criterion(F.normalize(axisangle_gt), F.normalize(axisangle_pred))  * 0.3    
            losses2["loss2_Raw_Ang"] = self.criterion(axisangle_gt, axisangle_pred) * 0.3
