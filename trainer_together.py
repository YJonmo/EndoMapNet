...
#                    losses["loss2_Raw"] = losses2["loss2_Raw"]            
    ...
    def process_batch2(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs2 = self.models["depth"](features[0])
        else:
            # Otherwise, we only feed the image with frame_id 0 through the depth encoder
            features = self.models["encoder"](inputs["color_aug", 0, 0])
            outputs2 = self.models["depth"](features)
        ...
        if self.use_pose_net:
            ...
            if self.num_pose_frames == 2:
                ...
                if self.opt.pose_model_type in ["separate_resnet", "posecnn"]:
                    ...
                    pose_inputs2 = [features[i] for i in self.opt.frame_ids if i != "s"]
...
#            if frame_id != "s":
#                i +=1   
#        abs_diff_trans = torch.zeros(i, requires_grad=True, dtype=torch.float32) 
#        abs_diff_trans = abs_diff_trans.to(device='cuda')
#        translation_pred = torch.zeros(2, 1, 3, requires_grad=False, dtype=torch.float32) 
...
#                abs_diff_trans[i] = (translation_pred[:,0,0,:] - translation_gt[:,:] + 0.0000001).abs().mean()
...
#            if frame_id != "s":
#                i +=1   
#        abs_diff_trans = torch.zeros(self.opt.batch_size, 3, i, requires_grad=True, dtype=torch.float64) 
#        abs_diff_trans = abs_diff_trans.to(device='cuda')
#        translation_pred = torch.zeros(self.opt.batch_size, 3, requires_grad=False, dtype=torch.float64) 
#        translation_pred = translation_pred.to(device='cuda')
...
#            if frame_id != "s":
#                #axisangle_pred = outputs[("axisangle", 0, frame_id)]
#                translation_pred[:] = outputs2[("translation", 0, frame_id)][:,0,0,:]*100
#                #axisangle_gt = inputs[("axisangle", frame_id, 0)]
#                Sign = torch.sign(inputs[("translation", frame_id, 0)])
#                #translation_gt = (-1*Sign)*torch.log10(torch.abs(inputs[("translation", frame_id, 0)]))/100
#                translation_gt = inputs[("translation", frame_id, 0)]*(.01/1)
...
#                i += 1
        ...
        losses2 = {}
        losses2["loss2_Normal"]=self.criterion(F.normalize(translation_gt), F.normalize(translation_pred))  / (15 * frame_lengh)   
        ...
        losses2["loss2"] = (losses2["loss2_Normal"])#/frame_lengh# + losses2["loss2_Raw"])/frame_lengh
        #losses2 = outputs[("translation", 0, f_i)][:,0].mean()
    
        #print('losses2[loss2]: ' + str(losses2["loss2"].data))
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.use_pose=='1':
            ...
            if  torch.mean(inputs[("translation", self.opt.frame_ids[1], 0)]) == 0:     # if the ground truth pose was not available then use the loss of the preveious run
                losses2["loss2_Normal"] = self.last_loss.item() ;
                #losses2["loss2_Normal"] =  total_loss  ; 
            else:
                losses2["loss2_Normal"]=self.criterion(F.normalize(translation_gt),F.normalize(translation_pred))/(15*frame_lengh)       
                #losses2["loss2_Raw"] = self.criterion(translation_gt, translation_pred) 
                self.last_loss = (self.criterion(F.normalize(translation_gt),F.normalize(translation_pred))/(15*frame_lengh) + self.last_loss)/2 
...
#            print(self.last_loss)    
            ...
            print('Losses2: ' + str(losses2["loss2"]))
            #print('Reprojection loss: ' + str(total_loss.data))
            #print("loss2 Normal: " + str(losses2["loss2_Normal"]))              
            losses["loss_reprojection"] = total_loss
            losses["loss2_Normal"] = losses2["loss2"]
            #losses["loss2_Raw"] = losses2["loss2_Raw"]
