...
#        self.set_train()
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            for i, f_i in enumerate(self.opt.frame_ids[1:]):
                if f_i != "s":
                    self.pred_poses.append((outputs[("cam_T_cam", 0, f_i)]).cpu().detach().numpy())
#            if batch_idx == 10:
#                print((outputs[("cam_T_cam", 0, 1)]).cpu().detach().numpy())
        ...
        self.pred_poses = np.concatenate(self.pred_poses)
        ...
        np.save(save_path, self.pred_poses)
        save_path = os.path.join(self.opt.load_weights_folder, "poses_" + self.opt.split[:]+"_SupVis.npy")
    ...
    def process_batch(self, inputs):
        ...
        if self.use_pose_net:
            ...
            #outputs2 = self.predict_poses(inputs, features)
            print('###########################################Output###############################')
            #print(outputs2)
...
#                #translation_gt = Sign*torch.log(torch.abs(inputs[("translation", frame_id, 0)]))/100
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type in ["separate_resnet", "posecnn"]:
                ...
                if self.opt.pose_model_type == "separate_resnet":
                    #pose_feats = {f_i: inputs[ "color_aug", f_i, 0] for f_i in self.opt.frame_ids}
                    #pose_inputs = []
                    #for I2 in self.opt.frame_ids:
                    #    if I2 != 's':
                    #        pose_inputs.append(pose_feats[I2])
                          
                    #pose_inputs = [pose_feats[-2], pose_feats[-1], pose_feats[0], pose_feats[1], pose_feats[2]]
                    #pose_inputs = [self.models["pose_encoder"](torch.cat(pose_inputs, 1))]
