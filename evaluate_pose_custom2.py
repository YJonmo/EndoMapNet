...
#                print((outputs[("cam_T_cam", 0, 1)]).cpu().detach().numpy())
            ...
            for key, ipt in inputs.items():
                ...
                #outputs[key].cpu().detach().numpy()
                print('key                  :')
                print(key)
                #print(outputs[key])
                for i, f_i in enumerate(self.opt.frame_ids[1:]):
                    if f_i != "s":
                        #self.pred_poses.append(transformation_from_parameters(
                        #        outputs[("axisangle", 0, f_i)], [("translation", 0, f_i)]).cpu().numpy())
    #                    outputs[("cam_T_cam", 0, f_i)] = transformation_from_parameters(
    #                            outputs[("axisangle", 0, f_i)], [("translation", 0, f_i)])
                        outputs[("cam_T_cam", 0, f_i)] = transformation_from_parameters(
                            outputs[("axisangle", 0, f_i)][:,i], outputs[("translation", 0, f_i)][:,i])
                        
                        self.pred_poses.append(outputs[("cam_T_cam", 0, f_i)].cpu().detach().numpy()) 
            #print(outputs[("axisangle", 0, 0)].cpu().numpy())
