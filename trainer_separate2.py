...
#                i += 1
        ...
        for frame_id in (self.opt.frame_ids[1:]):
            if frame_id != "s":
                i +=1   
        translation_gt = torch.zeros(self.opt.batch_size, 3, i, requires_grad=True, dtype=torch.float64) 
        translation_gt = translation_gt.to(device='cuda')
        translation_pred = torch.zeros(self.opt.batch_size, 3, i, requires_grad=False, dtype=torch.float64) 
        translation_pred = translation_pred.to(device='cuda')
        #abs_diff_axang = torch.zeros(i, dtype=torch.float64) 
        i=0
        for frame_id in (self.opt.frame_ids[1:]):
            if frame_id != "s":
                #axisangle_pred = outputs[("axisangle", 0, frame_id)]
                translation_pred[:, :, i] = outputs2[("translation", 0, frame_id)][:,0,0,:]*100
                #axisangle_gt = inputs[("axisangle", frame_id, 0)]
                Sign = torch.sign(inputs[("translation", frame_id, 0)])
                #translation_gt = (-1*Sign)*torch.log10(torch.abs(inputs[("translation", frame_id, 0)]))/100
                translation_gt[:, :, i] = inputs[("translation", frame_id, 0)]*(.01/1)
        ...
        coeff = torch.zeros(1, requires_grad=True, dtype=torch.float32)  
        ...
        print('losses2[loss2]: ' + str(losses2["loss2"]))
